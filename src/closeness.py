...
SONGS_FILE = "src/songs.csv"

def process_chunk(chunk, model, user_index):
    chunk.index = chunk["Title"]
    chunk_data = chunk.drop(columns=["Title"])  # Remove the title column for similarity calculations
...
user_index = 0  # The index of the user/song to find recommendations for
