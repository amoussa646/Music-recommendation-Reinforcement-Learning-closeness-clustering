...
def recommend_song(user_features, Q):
    action = np.argmax(Q[user_features])
    return songs.iloc[action]['track_name'], songs.iloc[action]['artists']

# Test the recommendation after training
recommended_song, artists = recommend_song(user_features, Q1)  # or Q2

print(f"Recommended song: {recommended_song} by {artists}")
